...
class OptionScore(Base):
    ...
    option_id: Mapped[UUID] = mapped_column(ForeignKey("options.id", ondelete="CASCADE"), nullable=False)
    ...
    traveler_type = relationship("TravelerType", back_populates="option_scores")
    
    __table_args__ = (
        UniqueConstraint('option_id', 'traveler_type_id', name='uq_option_travelertype'),
        CheckConstraint("score >= -10 AND score <= 10", name="check_score_range"),
    )

    def __repr__(self):
        return f"<OptionScore(option_id={self.option_id}, traveler_type_id={self.traveler_type_id}, score={self.score})>"
